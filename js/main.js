...
let services = $('.add').offset().top;
let logoH = heads.position().top;

$(window).scroll(function() {
	let winScroll = $(this).scrollTop(); // На сколько проскроллили сраницу (число)

	if (winScroll > services - logoH) { // Если проскроллили меньше 1000px от начала страницы
...
if($('.slider-client').length>0) {
	...
	$('.slider-client').slick('setPosition');
}
